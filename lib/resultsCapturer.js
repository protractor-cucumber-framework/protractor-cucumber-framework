...
switch (cucumberVersion) {
  ...
  case 7:
    Cucumber.After(runAfterEachScenario);
    class ResultsCapturerFormatter extends Cucumber.Formatter {
      constructor(options) {
        super(options);
        setup();
      }
      ...
      async finished() {
        computeResultsFromEventDataCollector(this.eventDataCollector.getTestCaseAttempts());
      }
    }
...
function computeResultsFromEventDataCollector(testCaseAttempts) {
  for (const testCaseAttempt of testCaseAttempts) {
    for (const stepResultKey in testCaseAttempt.stepResults) {
      const stepResult = testCaseAttempt.stepResults[stepResultKey];
      const stepDefinitionDetails = testCaseAttempt.testCase.testSteps.find(ts => ts.id === stepResultKey);
      const step = stepDefinitionDetails ? testCaseAttempt.pickle.steps.find(s => s.id === stepDefinitionDetails.pickleStepId) : undefined;
      const stepNodeId = step ? step.astNodeIds[0] : undefined;
      const feature = testCaseAttempt.gherkinDocument.feature;
      const featureChild = step && feature ? feature.children.find(c => c.scenario.steps.find(s => s.id === stepNodeId)) : undefined;
      const scenario = featureChild ? featureChild.scenario : undefined;
      const stepDetails = scenario ? scenario.steps.find(s => s.id === stepNodeId) : undefined;
      commonStepFinished({
        status: stepResult.status,
        duration: stepResult.duration,
        exception: stepResult.message,
        uri: testCaseAttempt.gherkinDocument.uri,
        line: stepDetails ? stepDetails.location.line : undefined,
        column: stepDetails ? stepDetails.location.column : undefined,
        isHidden: false,
        keyword: stepDetails ? stepDetails.keyword : undefined,
        stepName: step ? step.text : undefined,
        scenarioName: testCaseAttempt.pickle.name,
        featureName: feature ? feature.name : undefined,
        attemptNumber: testCaseAttempt.attempt,
        progress: undefined
      });
      // last step of scenario
      if (scenario && scenario.steps.length && scenario.steps[scenario.steps.length - 1].id === stepNodeId) {
        commonScenarioFinished(scenario.name);
      }
    }
  }
}
