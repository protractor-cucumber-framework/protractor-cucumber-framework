...
switch (cucumberVersion) {
  ...
  case 7: {
    ...
    class ResultsCapturerFormatter extends Cucumber.Formatter {
      ...
      async finished() {
        computeResultsFromEventDataCollector(
...
function computeResultsFromEventDataCollector(testCaseAttempts) {
  for (const testCaseAttempt of testCaseAttempts) {
    for (const stepResultKey in testCaseAttempt.stepResults) {
      ...
      const stepDefinitionDetails = testCaseAttempt.testCase.testSteps.find(
        ts => ts.id === stepResultKey
      );
      const step = stepDefinitionDetails
        ? testCaseAttempt.pickle.steps.find(
            s => s.id === stepDefinitionDetails.pickleStepId
          )
        : undefined;
      ...
      const stepDetails = scenario
        ? scenario.steps.find(s => s.id === stepNodeId)
        : undefined;
      commonStepFinished({
      ...
      if (
        ...
        scenario.steps[scenario.steps.length - 1].id === stepNodeId
      ) {
